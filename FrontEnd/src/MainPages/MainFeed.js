...
const MainFeed = () => {
  ...
  const [formData, setFormData] = useState({ receiver: '', picture: null, message: '' });
  const [decryptedMessage, setDecryptedMessage] = useState('');
  const [decryptedModalVisible, setDecryptedModalVisible] = useState(false);
  const [dropdownVisible, setDropdownVisible] = useState(false);

  const { logout } = useLogout();
  const { userViews, fetchFirstSix } = useGetSix();
  ...
  useEffect(() => {
    ...
    return () => document.body.classList.remove('mainfeed-body');
  }, [fetchFirstSix]);
  ...
  const closeForm = useCallback(() => {
    setFormVisible(false);
    setFormData({ receiver: '', picture: null, message: '' });
  ...
  const closeDecryptedModal = useCallback(() => {
    ...
    setDecryptedMessage('');
    document.querySelector('.massage-body').classList.remove('blurred');
    Swal.fire({ icon: 'success', title: 'Message Deleted!', text: 'The decrypted message has been deleted.', confirmButtonText: 'OK' })
  ...
  const toggleDropdown = useCallback(() => {
    setDropdownVisible((prev) => {
      ...
      document.querySelector('.massage-body').classList.toggle('blurred', isVisible);
      return isVisible;
    });
  }, []);
  ...
  return (
    <div className="mainfeed">
      ...
      <nav className="navbar">
        ...
        <i className="material-icons nav__icon new-message-btn" onClick={openForm}>edit_note</i>
        <div className="user-icon-container" onBlur={closeDropdown}>
          <i className="material-icons user-icon" onClick={toggleDropdown}>account_circle</i>
          {dropdownVisible && (
            <div className="profile-view-dropdown">
              <div className="profile-header">
                <div className="profile-icon">
                  {profileName ? <span className="profile-initial">{profileName.charAt(0).toUpperCase()}</span> : <i className="material-icons user-profile-icon">account_circle</i>}
                </div>
                <div className="profile-info">
                  <h3>{profileName}</h3>
                  <div className='phno'>
                    <b>Phone </b><b>number: </b><p>{profilePhone}</p>
                  </div>
          ...
          {userViews.length === 0 ? (
            ...
            userViews.map((user) => (
              <div className="message-item" key={user.messageId}>
                ...
                <button onClick={() => handleDecrypt(user.messageId)} className="decrypt-btn">Read</button>
              </div>
            ))
          )}
        </div>
      ...
      {formVisible && (
        <div className="form-popup1">
          <a onClick={closeForm} id="close" className="material-icons nav__icon">close</a><br />
          <form className="box3">
            <center>
              <h2>Compose New Message</h2><br />
              <input type="text" name="receiver" value={formData.receiver} onChange={handleInputChange} placeholder="Receiver" required />
              ...
              <button type="button" onClick={handleSendFormMessage}>Send Message</button>
            </center>
          </form>
        </div>
      )}
      ...
      {decryptedModalVisible && (
        <div className="form-popup2">
          <a onClick={closeDecryptedModal} id="close" className="material-icons nav__icon">close</a><br />
          <form className="box3">
            <center>
              <h2>Decrypted Message</h2><br />
              <p>{decryptedMessage}</p><br />
              <button type="button" onClick={closeDecryptedModal}>OK, Now Delete It</button>
            </center>
          </form>
        </div>
      )}
